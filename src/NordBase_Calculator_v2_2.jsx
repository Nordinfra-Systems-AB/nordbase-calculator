...
//
// BOLLARD depth + weight revised 2026-09-13 (Simon Gullberg): the Bollard/AC
// foundation has been redesigned with a deeper body (550mm, was
// ~502.92mm/19.8") and a revised laser-cut flange kerf (AC-specific: 8
// bridges × 3mm, was 4 × 3mm, for higher strength). Confirmed new weight is
// 8080g (17.81lb, was 16.3lb). depthIn and weightLb were updated below — see
// the comments on those fields for the exact source/conversion and for what
// is NOT modeled by any calc check (the flange/bridge detail itself).
// ---------------------------------------------------------------------------
const FOUNDATIONS = {
  BOLLARD: {
    key: "BOLLARD",
    ...
    // ---- Depth CHANGED 2026-09-13 (Simon Gullberg, redesigned AC/Bollard body) ----
    // New depth is 550mm; Simon confirmed all other dimensions are unchanged
    // ("i övrigt samma mått"). 550mm / 25.4 = 21.6535...", rounded to 21.65".
    // Previous value was 19.8" (502.92mm) — kept here only as a reference, not
    // used: depthIn: 19.8 (pre-2026-09-13).
    depthIn: 21.65, // 550mm, confirmed by Simon Gullberg 2026-09-13
    // weightLb CHANGED 2026-09-13 (Simon Gullberg): confirmed new weight for
    // the redesigned (550mm deep, 8-bridge flange) part is 8080g. 8080g /
    // 453.59237 = 17.8134...lb, rounded to 17.81lb. Previous confirmed weight
    // was 16.3lb (7393.6g), for the pre-redesign part. Verified the weight
    // change has negligible effect on the stability checks (governing
    // overturning-wind DCR moves from 19.61% to 19.58% at the new depth) —
    // consistent with Simon's own read ("gör inte så mycket för stabilitet").
    weightLb: 17.81, // 8080g, confirmed by Simon Gullberg 2026-09-13
    photoUrl: "/nordbase-bollard.png",
    hasCharger: false,
    hasAccessories: true,
    // ---- Confirmed structural data (Nordinfra_Master_USA_ASCE7_v6 — AC Fundament, 2026-08-21) ----
    // NOTE (2026-09-13): the laser-cut, bent-plate flange detail where the
    // adapter/post bolts on — the detail that failed first in all three
    // horizontal-load tests (Bollard/DCS/DCM), per
    // Horisontallasttest_Rådata_20260908 — is NOT modeled by the "Wall plate
    // bending" check below, or by any other check in this file. Simon
    // confirmed 2026-09-13 that the AC/Bollard flange is a DIFFERENT detail
    // from DCS/DCM's ("Flänsen till AC är annorlunda jämfört med DCS och
    // DCM") and gave AC-specific bridge counts: old AC flange = 4 bridges ×
    // 3mm = 12mm remaining material; new AC flange = 8 bridges × 3mm = 24mm
    // remaining material (ratio 2.0×, +100%). This is a DIFFERENT figure from
    // the 2026-09-11 "authoritative" 2×4mm→5×3mm/+87.5% record in the
    // Horisontallasttest doc — most likely because that record was for
    // DCS/DCM rather than AC, per today's product-specific clarification, but
    // this has been flagged to Simon rather than assumed. Because this
    // failure mode isn't modeled by any check here at all, there is nothing
    // in the calc engine to update for the bridge-count change itself — only
    // depth and weight (this file) feed into the existing checks.
